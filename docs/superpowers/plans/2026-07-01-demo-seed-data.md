...
**Goal:** Build a small, complete, idempotent `supabase/seed.sql` demo dataset for animals, sponsor animals, adoption cases, adopters, supporters/donors, donations, payments, receipts, and coordinator workflows.

**Architecture:** Use a single SQL seed file wrapped in repeatable, fixed-ID upserts. Resolve coordinator status IDs by `(category, key)` at insert time so the seed follows migration-created statuses instead of hard-coding status UUIDs. Keep the seed domain-only and do not touch Supabase Auth or `admin_user`.

**Tech Stack:** Supabase/Postgres SQL, existing HKSCDA migrations, fixed UUID demo records, Bun/Supabase CLI for verification.
...
- Create: `supabase/seed.sql`
  - Responsibility: idempotent local/demo seed data only.
  - Sections: reference lookups, animals, supporters/adopters, donations/payments/receipts, adoption/coordinator workflow, report audit rows.
- No TypeScript files should change.
- No admin/auth user files should change.

## Fixed Demo IDs

Use these prefixes consistently:

- Reference data: `20000000-*`, `21000000-*`, `22000000-*`, `23000000-*`
- Animals: `10000000-*`
- Supporters: `30000000-*`
- Consents: `32000000-*`
- Adopters: `31000000-*`
...
- Matches: `52000000-*`
- Follow-ups: `53000000-*`
- Successful adoptions: `54000000-*`
- Audit rows: `90000000-*`

---

### Task 1: Create Seed Skeleton, Reference Data, And Animals

**Files:**
- Create: `supabase/seed.sql`

- [ ] **Step 1: Write the seed skeleton and first dataset**
...
- [ ] **Step 2: Verify the seed file is visible to git**
...
Expected: `?? supabase/seed.sql` appears.

- [ ] **Step 3: Commit the first seed slice**

Run:

```bash
git add supabase/seed.sql
git commit -m "chore: seed demo animals"
```

Expected: commit succeeds with `supabase/seed.sql` created.

---

### Task 2: Add Supporters, Roles, Consents, And Adopter Profiles

**Files:**
- Modify: `supabase/seed.sql`

- [ ] **Step 1: Insert supporter/adopter SQL before `commit;`**

Open `supabase/seed.sql` and insert this block immediately before the final `commit;`:
...
- [ ] **Step 2: Confirm no admin auth tables are touched**

Run:

```bash
rg -n "admin_user|auth\\.users|auth\\.admin|createUser|inviteUser" supabase/seed.sql
```

Expected: no output.

- [ ] **Step 3: Commit supporter/adopter seed data**

Run:

```bash
git add supabase/seed.sql
git commit -m "chore: seed demo supporters"
```
...
### Task 3: Add Donation, Payment, And Receipt Demo Data

**Files:**
- Modify: `supabase/seed.sql`

- [ ] **Step 1: Insert finance SQL before `commit;`**
...
- [ ] **Step 2: Check finance status values**

Run:

```bash
rg -n "'refunded'|'voided'|, 'void'," supabase/seed.sql
```

Expected: no output. Receipt rows use `issued`; `voided_at` and `voided_by` schema column names are allowed.

- [ ] **Step 3: Commit finance seed data**

Run:

```bash
git add supabase/seed.sql
git commit -m "chore: seed demo donations"
```
...
### Task 4: Add Adoption Cases, Coordinator Workflow, And Report Audit Rows

**Files:**
- Modify: `supabase/seed.sql`

- [ ] **Step 1: Insert adoption workflow SQL before `commit;`**
...
- [ ] **Step 2: Confirm status lookups are dynamic**

Run:

```bash
rg -n "status_id = '|outcome_status_id = '|coordinator_status" supabase/seed.sql
```

Expected: status assignments use `select id from public.coordinator_status where category = ... and key = ...`; no hard-coded coordinator status UUID is used.

- [ ] **Step 3: Commit adoption workflow seed data**

Run:

```bash
git add supabase/seed.sql
git commit -m "chore: seed demo adoption workflow"
```
...
### Task 5: Validate Seed Shape And Idempotency

**Files:**
- Verify: `supabase/seed.sql`

- [ ] **Step 1: Check that only approved files changed**
...
Expected: clean except unrelated pre-existing untracked files such as `AGENTS.md`.

- [ ] **Step 2: Run basic text checks**

Run:

```bash
rg -n "truncate|delete from|admin_user|auth\\.users|auth\\.admin|createUser|inviteUser" supabase/seed.sql
```

Expected: no output.

- [ ] **Step 3: Check expected demo row counts in the file**

Run:

```bash
rg -c "10000000-0000-4000-8000-00000000000[1-8]" supabase/seed.sql
rg -c "30000000-0000-4000-8000-00000000000[1-6]" supabase/seed.sql
rg -c "40000000-0000-4000-8000-00000000000[1-6]" supabase/seed.sql
rg -c "51000000-0000-4000-8000-00000000000[1-5]" supabase/seed.sql
```

Expected: each command prints a number greater than or equal to the number of records in that domain because IDs appear in relationships as well as inserts.

- [ ] **Step 4: Validate against a local Supabase database**
...
Expected: migrations and `supabase/seed.sql` complete without SQL errors. If the Supabase CLI is unavailable or local Supabase is not configured, do not run against production; record in the final implementation summary that local database execution was not run.

- [ ] **Step 5: Re-run the seed to prove idempotency**
...
Expected: both resets complete without duplicate-key or foreign-key failures.

- [ ] **Step 6: Commit validation notes only if a file changed**
...
Expected: no new tracked changes. Do not commit generated Supabase local files.
...
Expected: zero lint errors. Existing Fast Refresh warnings are acceptable if unchanged.
...
Expected: build passes. Existing Vite/Nitro module directive warnings are acceptable if unchanged.
...
Expected: branch contains the seed commits and no unintended tracked changes.
