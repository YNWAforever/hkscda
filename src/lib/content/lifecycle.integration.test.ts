...
async function createFixture() {
  const [row] =
    await db`select public.create_content_revision_with_audit(${actor}::uuid, ${{ slug: `cms-${crypto.randomUUID()}`, type: "event", title: "Synthetic review fixture", summary: "Disposable local CMS acceptance fixture" }}::jsonb) as result`;
...
async function addCover(item: Result) {
  const [row] =
    await db`select public.mutate_content_revision_with_audit(${actor}::uuid, ${item.content_id}::uuid, ${item.version}, 'create_media', ${{ storageBucket: "content-media", storagePath: `${item.content_id}/synthetic.jpg`, altText: "Synthetic fixture", isCover: true }}::jsonb) as result`;
...
describe.skipIf(!enabled)("CMS lifecycle isolated database acceptance", () => {
  ...
  test("two writers using the same parent version produce one winner", async () => {
    ...
    const outcomes = await Promise.allSettled(
      ["First", "Second"].map(
        (title) =>
          db`select public.mutate_content_revision_with_audit(${actor}::uuid,${item.content_id}::uuid,${item.version},'save_content',${{ title }}::jsonb)`,
      ),
    );
  ...
  test("publication replay is stable and changed payload conflicts", async () => {
    ...
    await expectRejection(publish({ ...item, version: item.version + 1 }, key));
    const [count] =
  ...
  test("draft slug edits do not release the currently published URL", async () => {
    ...
    await db`select public.mutate_content_revision_with_audit(${actor}::uuid,${first.content_id}::uuid,${live.version},'save_content',${{ slug: `cms-new-${crypto.randomUUID()}` }}::jsonb)`;
    const second = await createFixture();
    const [saved] =
      await db`select public.mutate_content_revision_with_audit(${actor}::uuid,${second.content_id}::uuid,${second.version},'save_content',${{ slug: original.slug }}::jsonb) as result`;
    const ready = await addCover(saved.result as Result);
    await expectRejection(publish(ready, crypto.randomUUID()));
    const [after] =
  ...
  test("concurrent publication of historical identical slugs has one winner", async () => {
    ...
    const move = async (item: Result, slug: string) => {
      const [row] =
        await db`select public.mutate_content_revision_with_audit(${actor}::uuid,${item.content_id}::uuid,${item.version},'save_content',${{ slug }}::jsonb) as result`;
      return row.result as Result;
    };
  ...
  test.each([
    ...
    await db`insert into public.content_revision(id,content_item_id,version,operation,authoring_snapshot,public_snapshot,created_by) values(${invalidId}::uuid,${item.content_id}::uuid,0,'synthetic_invalid_snapshot',${source.authoring_snapshot}::jsonb,${snapshot}::jsonb,${source.created_by}::uuid)`;
    await expectRejection(publish({ ...item, revision_id: invalidId }, crypto.randomUUID()));
    const [after] =
  ...
  test("media finalization rejects an update removed from current authoring by restore", async () => {
    ...
    await expectRejection(
      db`select public.mutate_content_revision_with_audit(${actor}::uuid,${item.content_id}::uuid,${(restored.result as Result).version},'create_media',${{ storyUpdateId: updated.child_id, storageBucket: "content-media", storagePath: `${item.content_id}/removed.jpg`, altText: "Removed update" }}::jsonb)`,
    );
  });
