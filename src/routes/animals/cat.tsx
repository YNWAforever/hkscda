...
function CatListingPage() {
  ...
  if (isError) {
    return (
      <PublicStateShell
        ...
        action={<a href="/animals/cat" className="btn-primary min-h-11 px-5">重新整理</a>}
      />
    );
  }
  ...
  return (
    <main className="container-wide px-4 py-10 sm:px-6 lg:px-8">
      <h1 className="text-3xl font-bold text-[var(--color-text)]">待領養貓貓</h1>
      <p className="mt-3 text-[var(--color-text-muted)]">查看目前可申請領養的貓貓，了解牠們的需要。</p>
      <div className="mt-8">
        <AnimalGrid animals={data?.animals ?? []} total={data?.total ?? 0} page={page} ageFilter={filter as AgeFilter} pageSize={PAGE_SIZE} animalLabel="貓" />
      </div>
    </main>
  );
}