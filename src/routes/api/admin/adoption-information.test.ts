...
function setup(options: { unauthorized?: boolean; conflict?: boolean } = {}) {
  ...
  const service = {
    async listAdmin() { return { resource: "fees", items: [], total: 0, page: 1, pageSize: 25 }; },
    async upsertFee() { if (options.conflict) throw new AdoptionInformationConflictError("Fee order conflicts"); return { id: "fee-1" }; },
    async upsertEstate() { return { id: "estate-1" }; },
    async deleteEstate() {},
  };
  return {
    authCalls,
...
describe("adoption information admin handlers", () => {
  test("requires staff/admin auth and returns no-store request-id JSON", async () => {
    ...
    const response = await handlers.listAdmin({ request: new Request("https://example.test/api/admin/adoption-information") });
  ...
  test("preserves auth status and maps safe conflicts", async () => {
    const unauthorized = await setup({ unauthorized: true }).handlers.listAdmin({ request: new Request("https://example.test/api/admin/adoption-information") });
    ...
    const conflict = await setup({ conflict: true }).handlers.upsert({
      request: new Request("https://example.test/api/admin/adoption-information", {
        ...
        body: JSON.stringify({ resource: "fee", input: { animalType: "dog", itemName: "Mongrel 唐狗", priceHkd: "0", sortOrder: 1, isPublished: true } }),
      }),
    });
